...
contract WorkUrFreedomContract {
    ...
    mapping (address => uint[81]) private userData;
    // 0:       existance
    // 1-9:     demographics    // 9 questions
    // 10-18:   experience      // 9 questions
    // 19-80:   skills          // 62 skills
    ...
    function exists(uint[81] memory arr) private pure returns(bool) {
        if (arr[0] == 1) {
            return true;
        }
        return false;
    ...
    function addSkillsAnwsers(address userAddr, uint[62] memory answers) public {
        // Check if the sender of this transaction is the owner
        require(isOwner(msg.sender), "Permission Denied! Only the owner can perform this operation");
    ...
    function addUserData(address userAddr, uint[80] memory answers) public {
        // Check if the sender of this transaction is the owner
        require(isOwner(msg.sender), "Permission Denied! Only the owner can perform this operation");
    ...
    function getSkillsData() public view returns(uint[62] memory) {

        // Check if the address is stored in the map
        require(exists(userData[msg.sender]), "User does not exist...");

        // Else
        uint[62] memory returnArr;
        for (uint i = 19; i < 81; i++) {
    ...
    function getUserData() public view returns(uint[81] memory) {
    ...
    function getUsersData() public view returns(uint[81][] memory) {

       // Check if the sender of this call is the owner
        require(isOwner(msg.sender), "Permission Denied! Only the owner can perform this operation");

        uint[81][] memory ret = new uint[81][](usersNum);
