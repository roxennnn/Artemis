...
export const fetchMatchings = async (req, res) => {
  try {
    ...
    if (skillScores.length === 0) {
      throw new CustomError('Fetching skills data error.', 501);
    }

    const occupations = await getOccupationList(language);
    if (occupations.length === 0) {
      throw new CustomError('Fetching occupations data error.', 502);
    }
    ...
    return res.status(200).send({ scoredOccupations });
  } catch (err) {
    res
...
export const fetchOccupationDetail = async (req, res) => {
  try {
    ...
    details.categoryNames = occupation.category_names;
    details.categoryScores = categoryScores.sort((a, b) => {
      return b - a;
    });
...
export const fetchSkills = async (req, res) => {
  try {
    ...
    if (skillScores.length !== categories.length) {
      throw new CustomError('Skill array lengths are different.', 502);
    }
...
const getOccupationList = async (language) => {
  ...
  try {
    ...
    throw new CustomError(err.msg, 402);
  }
  return occupations;
};
...
const getOccupationDetail = async (oid, language) => {
  let occupation;
  try {
    occupation = await Occupation.findOne({
      OID: oid,
      language: language,
    }).exec();
  } catch (err) {
